// kaldifst/csrc/kaldi-fst-io-inl.h
//
// This file is copied/modified from
// https://github.com/kaldi-asr/kaldi/blob/master/src/fstext/kaldi-fst-io-inl.h

// Copyright 2009-2011  Microsoft Corporation
//           2012-2015  Johns Hopkins University (Author: Daniel Povey)
//                2013  Guoguo Chen

#ifndef KALDIFST_CSRC_KALDI_FST_IO_INL_H_
#define KALDIFST_CSRC_KALDI_FST_IO_INL_H_
...
void WriteFstKaldi(std::ostream &os, bool binary, const VectorFst<Arc> &t) {
  ...
  if (binary) {
    ...
    if (os.fail())
      KALDIFST_ERR << "Stream failure detected writing FST to stream";
  ...
  if (!ok) {
    KALDIFST_ERR << "Error writing FST to stream";
  }
}
...
void ReadFstKaldi(std::istream &is, bool binary, VectorFst<Arc> *fst) {
  ...
  if (binary) {
    ...
    VectorFst<Arc> *ans =
        VectorFst<Arc>::Read(is, FstReadOptions(std::string("[unknown]")));
    if (ans == NULL) {
      KALDIFST_ERR << "Error reading FST from stream.";
    }
    *fst = *ans;  // shallow copy.
    ...
    using kaldifst::ConvertStringToInteger;
    using kaldifst::SplitStringToIntegers;
    using std::string;
    using std::vector;
    fst->DeleteStates();
    string line;
    ...
    while (std::getline(is, line)) {
      ...
      kaldifst::SplitStringToVector(line, separator.c_str(), true, &col);
      if (col.size() == 0) break;  // Empty line is a signal to stop, in our
      // archive format.
      if (col.size() > 5) {
        KALDIFST_ERR << "Bad line in FST: " << line;
      }
      StateId s;
      ...
      if (!ok) KALDIFST_ERR << "Bad line in FST: " << line;
    }
  }
}
...
bool VectorFstTplHolder<Arc>::Read(std::istream &is) {
  ...
  int c = is.peek();
  if (c == -1) {
    KALDIFST_WARN << "End of stream detected reading Fst";
    return false;
...
#endif  // KALDIFST_CSRC_KALDI_FST_IO_INL_H_
