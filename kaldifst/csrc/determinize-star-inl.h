// kaldifst/csrc/determinize-star-inl.h
//
// This file is copied/modified from
// https://github.com/kaldi-asr/kaldi/blob/master/src/fstext/determinize-star-inl.h
...
class StringRepository {
 ...
 public:
  ...
  KALDIFST_DISALLOW_COPY_AND_ASSIGN(StringRepository);
...
class DeterminizerStar {
 ...
 public:
  KALDIFST_DISALLOW_COPY_AND_ASSIGN(DeterminizerStar);
    ...
    while (!Q_.empty()) {
      ...
      if (max_states_ > 0 && output_arcs_.size() > max_states_) {
        if (allow_partial_ == false) {
          KALDIFST_ERR << "Determinization aborted since passed " << max_states_
                       << " states";
        } else {
          KALDIFST_WARN << "Determinization terminated since passed "
                        << max_states_
      ...
      if (this_final_weight != Weight::Zero()) {
        if (!is_final) {  // first final-weight
          ...
          if (final_string != elem.string) {
            KALDIFST_ERR << "FST was not functional -> not determinizable";
          }
          final_weight =
...
void DeterminizerStar<F>::EpsilonClosure::GetEpsilonClosure(
  ...
  while (!queue_.empty()) {
    ...
    if (max_states_ > 0 && counter++ > max_states_) {
      KALDIFST_ERR << "Determinization aborted since looped more than "
...
void DeterminizerStar<F>::Debug() {
  ...
  KALDIFST_WARN << "Debug function called (probably SIGUSR1 caught)";
  ...
  if (output_arcs_.size() <= 2) {
    KALDIFST_ERR << "Nothing to trace back";
  }
  ...
  while (cur_state != 0 && cur_state != kNoStateId) {
    ...
    KALDIFST_ASSERT(i != output_arcs_[last_state].size());  // Or fell off loop.
    cur_state = last_state;
  }
  if (cur_state == kNoStateId)
    KALDIFST_WARN << "Traceback did not reach start state "
...
#endif  // KALDIFST_CSRC_DETERMINIZE_STAR_INL_H_
