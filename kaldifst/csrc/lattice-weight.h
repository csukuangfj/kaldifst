// kaldifst/csrc/lattice-weight.h
// Copyright 2009-2012  Microsoft Corporation
//                      Johns Hopkins University (author: Daniel Povey)
// This file is copied/modified from
// https://github.com/kaldi-asr/kaldi/blob/master/src/fstext/lattice-weight.cc
...
// Declare weight type for lattice...  has two
...
class LatticeWeightTpl {
 ...
 private:
  T value1_;  // usually graph cost
  T value2_;  // usually acoustic cost
};
...
class CompactLatticeWeightTpl {
 public:
  ...
  std::istream &Read(std::istream &strm) {
    ...
    if (sz < 0) {
      KALDIFST_WARN << "Negative string size!  Read failure";
      strm.clear(std::ios::badbit);
      return strm;
    }
    string_.resize(sz);
...
inline CompactLatticeWeightTpl<WeightType, IntType> Divide(
  ...
  if (div == DIVIDE_LEFT) {
    if (!std::equal(v2b, v2e,
      ...
      KALDIFST_ERR << "Cannot divide, data mismatch";
    }
    ...
    KALDIFST_ERR << "Cannot divide CompactLatticeWeightTpl with DIVIDE_ANY";
  }
